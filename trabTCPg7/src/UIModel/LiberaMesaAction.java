	...
	public void execute ()
	{
		...
		if (liberaMesa.isEmpty()){
			...
			for (int m = 0; m < liberaMesa.size(); m++) {
				System.out.println(liberaMesa.get(m).getSetor().getNome()
							+" / "+liberaMesa.get(m).getCodigoMesa());
			}
			...
			for (int mesa = 0; mesa < liberaMesa.size(); mesa++) {
				...
				if ( (mesaParaLiberar.getSetor().getNome().equals(setor)) 
								&& (mesaParaLiberar.getCodigoMesa().equals(codMesa)) ){
