		...
		try {
			...
			if (mesasParaXCapacidade.isEmpty()){
				...
				for (int m = 0; m < mesasParaXCapacidade.size(); m++) {
					System.out.println(mesasParaXCapacidade.get(m).getSetor().getNome()
								+" / "+mesasParaXCapacidade.get(m).getCodigoMesa()+" -- Capacidade: "+
												mesasParaXCapacidade.get(m).getCapacidade());
				}
